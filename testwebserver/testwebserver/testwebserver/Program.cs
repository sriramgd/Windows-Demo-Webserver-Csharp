...
namespace MyServer
{
    ...
    class Program
    {
        ...
        static string serverstring = "CMTServer/1.63";
        ...
        static void HTTPToCGI(string req, string http, string cgi)
        {
            ...
            if (reg.Success)
            {
                Environment.SetEnvironmentVariable(cgi, reg.Groups[1].Value);
            }
            else
            {
                Environment.SetEnvironmentVariable(cgi, "");
            }
        }
        ...
        static Process PrepareCGI(string method, string path, string req, TcpClient client)
        {
            ...
            Environment.SetEnvironmentVariable("SERVER_NAME", host);
            Environment.SetEnvironmentVariable("SERVER_PORT", port.ToString());
            Environment.SetEnvironmentVariable("SERVER_SOFTWARE", serverstring);

            Environment.SetEnvironmentVariable("REMOTE_ADDR", client.Client.RemoteEndPoint.ToString().Split(':')[0]);
            Environment.SetEnvironmentVariable("REQUEST_METHOD", method);
            Environment.SetEnvironmentVariable("SCRIPT_NAME", script.Replace(wwwroot, ""));
            ...
            {
                Environment.SetEnvironmentVariable("REDIRECT_STATUS", "200");
                Environment.SetEnvironmentVariable("SCRIPT_FILENAME", script);
        ...
        static void HandleRequest(TcpClient c)
        {
            if (loglevel >= 3)
            {
                var info = String.Format("{0} - {1} connected.", DateTime.Now, c.Client.RemoteEndPoint);
            ...
            try
            {
                ...
                if (loglevel >= 3)
                {
                    var info = String.Format("{0} - {1}: \"{2}\"", DateTime.Now, c.Client.RemoteEndPoint, reqline[0].Substring(0, reqline[0].Length - 1));
        ...
        private static string StartRewrite(string path1)
        {
            ...
            foreach (string line in rewrite)
            {
                ...
                catch
                {
                    if (loglevel >= 1)
                    {
                        var err = String.Format("{0} - ERROR IN FILTER: \"{1}\"", DateTime.Now, line);
        ...
        private static void ProcessGET(TcpClient c, NetworkStream ns, StreamWriter w, string req, string[] reqline, ref string path1, ref string path2)
        {
            ...
            if (!isfile)
            {
                GenerateList(ns, w, reqline, path1, path2);
            }
            else
            {
                // file
        ...
        private static void GenerateList(NetworkStream ns, StreamWriter w, string[] reqline, string path1, string path2)
        {
        ...
        private static void ProcessHttpRequests()
        {
            ...
            {
                var err = String.Format("{0} - ERROR: {1}", DateTime.Now, e.Message);
                if (loglevel >= 1)
                {
                    Console.WriteLine(err);
                    log.WriteLine(err);
